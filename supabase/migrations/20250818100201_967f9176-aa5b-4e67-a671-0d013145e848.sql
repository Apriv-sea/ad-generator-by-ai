-- Fix API key encryption and RLS security issues
...
CREATE POLICY "Users can view own and authorized templates" 
...
USING (
  ...
  -- Remove organization access unless user is verified member
);

-- 5. Strengthen profiles RLS to prevent email harvesting
...
-- 6. Add missing constraints and triggers for security
ALTER TABLE public.api_keys 
ADD CONSTRAINT api_keys_user_id_not_null CHECK (user_id IS NOT NULL);

-- 7. Add trigger to validate API key operations
...
-- 8. Fix search_path issues in existing functions
