...
class GoogleAuthService {
  ...
  constructor() {
    // Utiliser l'URL correcte basée sur l'environnement actuel
    ...
    console.log("Configuration OAuth Google:", {
  ...
  async handleCallback(code: string, state: string): Promise<boolean> {
    try {
      console.log("Traitement du callback OAuth:", { code: code?.substring(0, 10) + "...", state });
      
      // Valider l'état OAuth
      const storedState = secureStorageService.getSecureItem(this.STATE_STORAGE_KEY, { encrypt: true });
      if (!storedState || storedState !== state) {
        console.error("État OAuth invalide:", { storedState, receivedState: state });
      ...
      console.log("Authentification Google réussie:", { email: userInfo.email });
      ...
      if (error.message?.includes('redirect_uri_mismatch')) {
        toast.error("Erreur de configuration: URI de redirection incorrecte. Vérifiez votre configuration Google Cloud.");
      } else if (error.message?.includes('invalid_client')) {
        toast.error("Client ID invalide. Vérifiez votre configuration Google Cloud.");
      } else {
        toast.error("Erreur lors de la connexion à Google Sheets. Vérifiez votre configuration.");
      }
      return false;
    }
  }
